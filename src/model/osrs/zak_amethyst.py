...
class Amethyst(OSRSBot):
    ...
    def __bank(self, api_m: MorgHTTPSocket):
        ...
        self.mouse.click()
        api_m.wait_til_gained_xp(skill="Mining", timeout=12)
    ...
    def main_loop(self):
        ...
        while time.time() - start_time < end_time:
            amethyst_vein = self.get_all_tagged_in_rect(self.win.game_view, clr.PINK)
            bank_deposit = self.get_all_tagged_in_rect(self.win.game_view, clr.YELLOW)
            amethyst = api_m.get_first_occurrence(item_id=ids.AMETHYST)
            ...
            if api_s.get_is_inv_full():
                ...
                for rock in amethyst_vein:
                    if api_s.get_is_player_idle():
                        ...
                        self.log_msg("Player not idle, waiting...")
                        time.sleep(1)
                        break
